...
이 툰 셰이더는 광원(light source)의 위치와 강도를 걱정할 필요 없이 캐릭터 모델 속 다양한 부분의 형태를 강조하는 등 캐릭터 디자인에서 필수적인 모든 섀도우 요소들을 쉽게 만들어낼 수 있도록 구성되어있습니다. 또한 섀도우 색상 설정은 컬러 디자인을 담당하는 사람이 쉽게 섀도우을 만들어낼 수 있도록 해줍니다.

본 셰이더의 특별히 강력한 기능 중 하나는, 여러개의 광원을 사용할 필요 없이 셰이더 자체 내의 슬라이더를 사용하여 섀도우를 조절할 수 있다는 것입니다.
...
본 셰이더는 디자인에 필요한 고정 섀도우를 만들기 위한 2가지 옵션이 존재하는데, 각 섀도우에 캐스팅 포인트를 할당하는 **Position Map**과 조명에 따라 섀도우의 강도를 조정할 수 있는 **Shading Grade Map**이 있습니다.  
